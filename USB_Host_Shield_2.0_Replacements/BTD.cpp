...
void BTD::Initialize() {
        ...
        // Don't clear link key on Initialize — we need it to survive dongle re-enumeration
        // link_key_valid is only set false in the constructor
}
                ...
                switch(hcibuf[0]) { // Switch on event type
                        ...
                        case EV_INQUIRY_COMPLETE:
                                ...
                                if(inquiry_counter >= 5 && pairWithHIDDevice) {
                                        ...
                                        inquiry_counter = 0;
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR("\r\nHID inquiry round done, retrying..."), 0x80);
#endif
                                }
                                inquiry_counter++;
                                break;
...
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR(" — replying with stored key"), 0x80);
#endif
                                        hci_link_key_request_reply();
                                } else {
#ifdef DEBUG_USB_HOST
                                        Notify(PSTR(" — no key, negative reply"), 0x80);
#endif
...
#endif
                                                if(link_key_valid) {
                                                        // Reconnect with stored link key — enable encryption first.
                                                        // Wait for EV_ENCRYPTION_CHANGE before starting L2CAP.
...
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\nUser Passkey Request — keyboard must type passkey"), 0x80);
#endif
                                // We have DisplayOnly capability, so we should get notification instead
                                // But if we get request, send passkey negative reply
                                {
                                        uint8_t buf[9];
                                        buf[0] = 0x2F; buf[1] = 0x01 << 2; buf[2] = 0x06; // HCI_User_Passkey_Request_Negative_Reply
                        ...
                        case EV_LINK_KEY_NOTIFICATION:
                                // Store link key for reconnection
                                // hcibuf[2..7] = BD_ADDR, hcibuf[8..23] = Link Key, hcibuf[24] = Key Type
                                for(uint8_t i = 0; i < 6; i++)
                                ...
                                link_key_valid = true;
#ifdef DEBUG_USB_HOST
                                Notify(PSTR("\r\nLink key stored for "), 0x80);
                                for(int8_t i = 5; i > 0; i--) {
                                        D_PrintHex<uint8_t>(link_key_bdaddr[i], 0x80);
                                        Notify(PSTR(":"), 0x80);
                                }
                                D_PrintHex<uint8_t>(link_key_bdaddr[0], 0x80);
#endif
                                break;
...
#endif
                                        // Now safe to start L2CAP after reconnect with stored link key
...
void BTD::hci_link_key_request_reply() {
        hcibuf[0] = 0x0B; // HCI OCF = 0B (Link_Key_Request_Reply)
        hcibuf[1] = 0x01 << 2; // HCI OGF = 1
        hcibuf[2] = 0x16; // parameter length 22 (6 BD_ADDR + 16 link key)
        hcibuf[3] = disc_bdaddr[0]; // 6 octet bdaddr
        hcibuf[4] = disc_bdaddr[1];
        hcibuf[5] = disc_bdaddr[2];
        hcibuf[6] = disc_bdaddr[3];
...
void BTD::hci_io_capability_request_reply() {
        ...
        hcibuf[7] = disc_bdaddr[4];
        hcibuf[8] = disc_bdaddr[5];
        hcibuf[9] = 0x01; // DisplayOnly — enables SSP Passkey Entry for keyboards
        hcibuf[10] = 0x00; // OOB authentication data not present
        hcibuf[11] = 0x04; // MITM Required, Dedicated Bonding
